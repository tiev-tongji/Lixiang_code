...
class Bottleneck3D(nn.Module):
    def __init__(
        ...
        self.downsample2 = nn.Sequential(
            ...
            nn.Conv3d(planes, planes, kernel_size=(1,1,1), stride=(1,1,1), bias=False),
    ...
    def forward(self, x):
        ...
        if self.downsample is not None:
            ...
            # print('self.downsample：', residual.shape)
...
class Bottleneck3D_Down(nn.Module):
    def __init__(
        self,
        inplanes,
        planes,
        ...
        super(Bottleneck3D_Down, self).__init__()
    ...
    def forward(self, x):
        ...
        #print('out5.shape：', out5.shape)
        if self.downsample is not None:
            B, C, D, H, W = out5.shape

            residual = self.downsample(x)
            residual = Upsample_3D(residual, D, H, W)
            #print('self.downsample：', residual.shape)
