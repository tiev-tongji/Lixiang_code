...
def vis_nuscene():
    ...
    i = 1
    true = np.load("/ ... /400_true.npy", allow_pickle=True)
    true = true[i, :]

    voxels = true
    pre = np.load("/ ... /400_pre.npy", allow_pickle=True)
    mask = np.load("/ ... /400_mask.npy", allow_pickle=True)

    pre = pre[i,:]

    PRE = torch.tensor(pre)
    TRUE = torch.tensor(true)
    t_out = torch.unique(TRUE, return_counts=True)
    print('真值分布：', t_out)

    p_out = torch.unique(PRE, return_counts=True)
    print('预测值分布：', p_out)


    mask = mask.reshape(3,6,200,200,17)
    mask = mask[i, :]

    mask_0 = mask[0, :].reshape(200,200,16)
    mask_1 = mask[1, :].reshape(200,200,16)
    mask_2 = mask[2, :].reshape(200,200,16)
    mask_3 = mask[3, :].reshape(200,200,16)
    mask_4 = mask[4, :].reshape(200,200,16)
    mask_5 = mask[5, :].reshape(200,200,16)

    mask_all = np.logical_or((mask_0 == True), np.logical_or((mask_1 == True),np.logical_or((mask_2 == True),
                                np.logical_or((mask_3 == True),np.logical_or((mask_4 == True),(mask_5 == True))))))
    pre[~mask_all] = 18


    mask_17 = np.logical_and((true == 17), (true > 0))
    pre[mask_17] = 17
    voxels = pre
